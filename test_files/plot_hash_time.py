...
def parse_data(log_file):
    ...
    try:
        ...
        print("Please place the script in the same directory as 'with_precision_ns.txt'")
        sys.exit(1)
...
def plot_hash_performance(data_points, output_filename="hash_performance_plot.png"):
    """
    ...
    # Convert to more readable units (KB and ms)
    file_sizes_kb = [s / 1024 for s in file_sizes_bytes]
    times_ms = [t / 1000 for t in times_us]

    # --- Create the Plot ---
    
    # Set a professional style (e.g., 'seaborn-whitegrid', 'ggplot')
    plt.style.use('seaborn-v0_8-whitegrid')
    
    # Create a figure with a good size for presentation
    plt.figure(figsize=(10, 6))

    # Plot the data: line, markers, and a label
    plt.plot(file_sizes_kb, times_ms, marker='o', linestyle='-', 
             color='b', label='SHA-256 (Single-shot in TEE)')

    # Set the x-axis to a log scale (base 2)
    plt.xscale('log', base=2)
    
    # --- Customize Labels, Title, and Ticks ---
    
    # Title and axis labels
    plt.title('SHA-256 Hash Computation Performance', fontsize=16, fontweight='bold')
    plt.xlabel('File Size (KB)', fontsize=12)
    plt.ylabel('Pure Compute Time (ms)', fontsize=12)

    # Set explicit x-axis ticks to match our data points
    plt.xticks(file_sizes_kb, [f'{int(s)}' for s in file_sizes_kb])
    
    # Ensure x-axis ticks are not in scientific notation
    ...
    # Save the plot to a high-DPI file
...
if __name__ == "__main__":
    LOG_FILE = 'with_precision_ns.txt'
    parsed_data = parse_data(LOG_FILE)
    plot_hash_performance(parsed_data)
