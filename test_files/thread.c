...
/*
 * This function enables EL0 (TA) access to all performance counters:
 * 1. Generic Timer (CNTPCT_EL0, CNTFRQ_EL0) via CNTKCTL_EL1
 * 2. PMU Cycle Counter (PMCCNTR_EL0) via PMUSERENR_EL0 and PMCR_EL0
 */
static void enable_el0_performance_counters(void)
{
    uint64_t val;

    /* 1. Enable Generic Timer (for nanosecond wall-clock time) */
    
    /* Read the Kernel Control register */
    __asm__ volatile("mrs %0, cntkctl_el1" : "=r" (val));
    
    /* Set bit 0 (EL0PCTEN) to allow EL0 access to CNTPCT_EL0 */
    /* Set bit 1 (EL0FRQEN) to allow EL0 access to CNTFRQ_EL0 */
    val |= (1 << 0) | (1 << 1); 
    ...
    /* 2. Enable PMU (for pure CPU cycle counts) */
    
    /* Enable user-space (S-EL0) access to PMU registers */
    __asm__ volatile("msr pmuserenr_el0, %0" :: "r" (1UL));
    
    /* Read the PMU Control Register */
    __asm__ volatile("mrs %0, pmcr_el0" : "=r" (val));
    
    /* Set bit 0 (E, Enable) and bit 2 (C, Cycle Counter Reset) */
    val |= (1 << 0) | (1 << 2);
...
void thread_init_per_cpu(void)
{
	...
	thread_init_vbar(get_excp_vect());
        ...
        enable_el0_performance_counters();
